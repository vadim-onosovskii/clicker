...
open class MainActivity : AppCompatActivity() {
    ...
    private fun loadData() {
        val sharedPreferences = getSharedPreferences("sharedPrefs", Context.MODE_PRIVATE)
        variables.cnt = sharedPreferences.getLong("Count_KEY", 0.toLong())
        variables.lem_stand_price = sharedPreferences.getLong("LemStand_KEY", 100.toLong())
        variables.startUp_price = sharedPreferences.getLong("StartUp_KEY", 1000.toLong())
        variables.advAgency_price = sharedPreferences.getLong("AdvAgency_KEY", 12000.toLong())
        variables.twitter_price = sharedPreferences.getLong("Twitter_KEY", 150000.toLong())
        variables.google_price = sharedPreferences.getLong("Google_KEY", 1500000.toLong())
        variables.tourism_price = sharedPreferences.getLong("Tourism_KEY", 30000000.toLong())
        variables.underground_bar_price = sharedPreferences.getLong("Bar_KEY", 1000.toLong())
        variables.night_club_price = sharedPreferences.getLong("Club_KEY", 10000.toLong())
        variables.counterfuit_goods_price = sharedPreferences.getLong("Counterfuit_KEY", 100000.toLong())
        variables.casino_price = sharedPreferences.getLong("Casino_KEY", 1000000.toLong())
        variables.boardwalk_empire_price = sharedPreferences.getLong("Empire_KEY", 100000000.toLong())
        variables.timerrate = sharedPreferences.getLong("Timerrate_KEY", 10000.toLong())
        variables.is_timerrate_too_low = sharedPreferences.getBoolean("Is_Timerrate_Too_Low_KEY", false)
        variables.moneypersec = sharedPreferences.getLong("MoneyPerSec_KEY", 0.toLong())
        variables.moneyperclick = sharedPreferences.getLong("MoneyPerClick_KEY", 1.toLong())
    }
}
